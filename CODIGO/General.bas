...
'TODO : esto de ServerRecibidos no se podría sacar???
    ...
    frmCargando.Barra.Width = frmCargando.Barra.Width + 101
    ...
    frmCargando.Barra.Width = frmCargando.Barra.Width + 103
    
    '///////////////////////////// EDUARDITO //////////////////////////
    EngineRun = True
    frmMain.hlst.List(0) = "Tormenta de fuego"
    frmMain.hlst.List(1) = "Descarga Electrica"
    frmMain.hlst.List(2) = "Apocalipsis"
    frmMain.hlst.List(3) = "Inmovilizar"
    frmMain.hlst.List(4) = "Remover Paralisis"
    ...
    UserMap = 1
    handle = FreeFile()
    Open DirMapas & "Mapa" & UserMap & ".map" For Binary As handle
    Seek #1, 1
    Get #1, , tempint
    Close #1
    ...
    charlist(UserCharIndex).Active = 0
    charlist(UserCharIndex).priv = 0 'los dos somos dioses :O
    charlist(UserCharIndex).invisible = False
    charlist(UserCharIndex).nombre = "Eduardo"
     
     Call MakeChar(100, 56, 1, 3, 40, 40, 10, 0, 4)
    ...
    MihpMax = 300
    NPChpMax = 2000
    frmMain.ManaBar.Caption = MImana
    frmMain.HpBar.Caption = MIhp
    ...
    frmMain.ManaBar.Caption = MImana & "/" & MimanaMax
    frmMain.HpBar.Caption = MIhp & "/" & MihpMax

    frmMain.Hpshp.Width = (((MIhp + 1 / 100) / (MihpMax + 1 / 100)) * 94)
    frmMain.MANShp.Width = (((MImana + 1 / 100) / (MimanaMax + 1 / 100)) * 94)
...
'**************************************************************
    ...
    Call DibujarMiniMapa
    If Map = 1 Then
    Light.Create_Light 41, 54, 4, 255, 255, 100
    Light.Create_Light 41, 39, 4, 255, 255, 100
    
    Light.Create_Light 58, 39, 4, 255, 255, 100
    Light.Create_Light 58, 54, 4, 255, 255, 100
    
    Light.Create_Light 50, 46, 9, 250, 255, 255
...
Public Sub CrearNPCMago()
charlist(UserCharIndex + 1) = charlist(UserCharIndex)
charlist(UserCharIndex + 1).Criminal = 1
charlist(UserCharIndex + 1).nombre = "Bot 2.0"
Call MakeChar(101, 56, 201, 3, 43, 40, 1, 4, 6)
End Sub


Sub MisPasos()
With charlist(UserCharIndex).Pos
MapData(.X, .Y).Paso.Active = 1
MapData(.X, .Y).Paso.Grh = 35
MapData(.X, .Y).Paso.LifeTime = engine.FPS * 10
MapData(.X, .Y).Paso.Alpha = 200
MapData(.X, .Y).Paso.Head = charlist(UserCharIndex).Heading
End With
End Sub
...
MapData(.X, .Y).Paso.LifeTime = engine.FPS * 10
MapData(.X, .Y).Paso.Alpha = 200
MapData(.X, .Y).Paso.Head = charlist(UserCharIndex + 1).Heading
End With
End Sub

Sub YoGanador()
With charlist(UserCharIndex).Pos
MapData(.X, .Y).Ganador.Active = 1
MapData(.X, .Y).Ganador.Grh = 35
MapData(.X, .Y).Ganador.LifeTime = engine.FPS * 10
MapData(.X, .Y).Ganador.Alpha = 200
MapData(.X, .Y).Ganador.Head = charlist(UserCharIndex).Heading
MapData(.X, .Y).Ganador.Sube = 30
End With
End Sub
