Attribute VB_Name = "Particles"
    ...
    '*****************************************************************
    'More info: http://www.vbgore.com/CommonCode.Partic ... less_Reset" class="postlink" rel="nofollow" onClick="window.open(this.href);return false;
    '*****************************************************************
    Dim a As Single
    Dim X As Single
    Dim Y As Single
    ...
    Dim TargetA As Single
        ...
        If Effect(EffectIndex).BindToChar > 0 Then
            ...
            Else
                ...
                Effect(EffectIndex).GoToX = engine.Engine_TPtoSPX(charlist(Effect(EffectIndex).BindToChar).Pos.X) + 16
                Effect(EffectIndex).GoToY = engine.Engine_TPtoSPY(charlist(Effect(EffectIndex).BindToChar).Pos.Y)
        ...
        If Effect(EffectIndex).GoToX > -30000 Or Effect(EffectIndex).GoToY > -30000 Then
            If Effect(EffectIndex).GoToX <> Effect(EffectIndex).X Or Effect(EffectIndex).GoToY <> Effect(EffectIndex).Y Then
                ...
                'Calculate the angle
                TargetA = engine.Engine_GetAngle(Effect(EffectIndex).X, Effect(EffectIndex).Y, Effect(EffectIndex).GoToX, Effect(EffectIndex).GoToY) + 180
...
Private Sub Effect_Rain_Reset(ByVal EffectIndex As Integer, ByVal Index As Long, Optional ByVal FirstReset As Byte = 0)
    If FirstReset = 1 Then
        ...
        'The very first reset
        Effect(EffectIndex).Particles(Index).ResetIt -200 + (Rnd * (frmMain.ScaleWidth + 400)), Rnd * (frmMain.ScaleHeight + 400), Rnd * 5, 25 + Rnd * 12, 0, 0

    Else

        'Any reset after first
        Effect(EffectIndex).Particles(Index).ResetIt -200 + (Rnd * 1200), -15 - Rnd * 185, Rnd * 5, 25 + Rnd * 12, 0, 0
        If Effect(EffectIndex).Particles(Index).sngX < -20 Then Effect(EffectIndex).Particles(Index).sngY = Rnd * (frmMain.ScaleHeight + 400)
        If Effect(EffectIndex).Particles(Index).sngX > frmMain.ScaleWidth Then Effect(EffectIndex).Particles(Index).sngY = Rnd * (frmMain.ScaleHeight + 400)
        If Effect(EffectIndex).Particles(Index).sngY > frmMain.ScaleHeight Then Effect(EffectIndex).Particles(Index).sngX = Rnd * (frmMain.ScaleWidth + 400)
